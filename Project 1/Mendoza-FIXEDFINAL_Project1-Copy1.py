...
#(OPTIONAL) Load a CSV file 

#(OPTIONAL) Convert string column to float (encoding of some sort?)
...
def evaluate_algorithm(dataset, algorithm, n_fols, *args):
...
#Make a prediction given coefficients
...
#Iterate through our dataset trX

dataset = trX_array.tolist()
for row in dataset:
...
def_coefficients_sgd(train, l_rate, n_epoch):
